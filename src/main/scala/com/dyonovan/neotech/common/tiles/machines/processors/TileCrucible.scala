...
import com.dyonovan.neotech.registries.CrucibleRecipeManager
...
class TileCrucible extends MachineProcessor[ItemStack, FluidStack] with FluidHandler {
    ...
    override def getOutput(stack: ItemStack): FluidStack = {
        if(RecipeManager.getHandler[CrucibleRecipeManager](RecipeManager.Crucible).getOutput(stack).isDefined)
            RecipeManager.getHandler[CrucibleRecipeManager](RecipeManager.Crucible).getOutput(stack).get
        else
            null
    }
